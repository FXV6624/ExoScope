...
class TestUserModel:
    ...
    def test_user_gets_uuid_on_creation(self, db: Session):
        ...
        user = create_user(session=db, user_create=uc, hashed_password=get_password_hash(uc.password))
...
class TestExoplanetModel:
    ...
    def test_exoplanet_unique_constraint_allows_different_star(self, db: Session):
        ...
        count = db.exec(select(Exoplanet).where(Exoplanet.planet_name == "Planet-b")).all()
        assert len(count) == 2
