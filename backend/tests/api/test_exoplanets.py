...
def _insert_planet(db: Session, name: str, method: str = "Transit", year: int = 2010):
    p = Exoplanet(
        planet_name=name, host_star="Star", discovery_method=method, discovery_year=year
    )
    db.add(p)
    db.commit()
    db.refresh(p)
...
class TestGetExoplanets:
    ...
    def test_exoplanets_filters(
        ...
        response = client.get(
            f"{API}/exoplanets/?discovery_year=2020", headers=superuser_token_headers
        )
        assert response.status_code == 200
        data = response.json()
...
class TestGetExoplanetStats:
    def test_get_stats_returns_aggregated_data(
        ...
        response = client.get(
            f"{API}/exoplanets/stats", headers=superuser_token_headers
        )
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 3
...
class TestGetExoplanetById:
    def test_get_by_id_returns_planet(
        self, client: TestClient, superuser_token_headers: dict, db: Session
    ):
        p = _insert_planet(db, "MyPlanet")
        response = client.get(
            f"{API}/exoplanets/{p.id}", headers=superuser_token_headers
        )
        assert response.status_code == 200
    ...
    def test_get_by_id_not_found(
        ...
        response = client.get(
            f"{API}/exoplanets/{uuid.uuid4()}", headers=superuser_token_headers
        )
    ...
    def test_get_by_id_invalid_uuid(
        ...
        response = client.get(
            f"{API}/exoplanets/not-a-uuid", headers=superuser_token_headers
        )
        assert response.status_code == 404
