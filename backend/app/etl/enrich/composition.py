...
def calculate_composition(exoplanet: ExoplanetBase) -> CompositionResult:
    ...
    if radius is None:
        return CompositionResult(composition=PlanetComposition.UNKNOWN,confidence=0.0,)
...
def _classify_from_density(radius: float,density: float,) -> CompositionResult:
    ...
    for threshold in PLANET_CLASSIFICATIONS:
        ...
        if density >= threshold.min_density:
            return CompositionResult(composition=threshold.composition,confidence=threshold.density_confidence,)

    ice_giant = PLANET_CLASSIFICATIONS[-2]
    gas_giant = PLANET_CLASSIFICATIONS[-1]

    if radius >= ice_giant.max_radius:
        return CompositionResult(composition=gas_giant.composition,confidence=gas_giant.density_confidence,)

    return CompositionResult(composition=ice_giant.composition,confidence=ice_giant.density_confidence,)
...
def _classify_from_radius(radius: float) -> CompositionResult:
    ...
    for threshold in PLANET_CLASSIFICATIONS:
        ...
        if radius < threshold.max_radius:
            return CompositionResult(composition=threshold.composition,confidence=threshold.radius_confidence,)

    gas_giant = PLANET_CLASSIFICATIONS[-1]

    return CompositionResult(composition=gas_giant.composition,confidence=gas_giant.radius_confidence,)